...
class NetShell:
    def __init__(self, trc, psz = 64, bsz=64, ysz=32, latent_sz=32, indisc = 0, ydisc = 0, 
                 embedim = 0, feat = 1, prevfeat = 1, inplus = 0, pre_train = 1, 
                 recall_train = 0, pred_natural = 0):
        self.trc = trc
        self.p_seq_len = psz
        self.b_seq_len = bsz
        self.y_seq_len = ysz #ysz은 호출측에서 데이터에 bos,eos를 포함시키던 아니던 순수데이터에서
                             #<bos/eos> token 처리 공간 한개 1개가 추가된 길이로 설정해야 한다.
        self.x_seq_len = bsz - self.y_seq_len
        self.target_len = ysz 
        ...
        self.bgate_bsize = 1
        self.pred_natural = pred_natural #pred_natural이 1이면 anormal에서 호출되는 경우로서 예측값의 
        #끝 end token을 자르지 않고 또 ydisc가 1이어도 타입을 정수로 변경하지 않는다. anormal의 
        #auto regress는 토큰처리를 모델내에서 수행하고 외부에서는 토큰 길이 고려하지 않고 순수 데이터 
        #길이로만 취급하기때문, 그러나 generic은 ysz의 길이가 token한개가 포함된 길이이고 예측값의 
        #마지막에 end token이 있으므로 anormal에서는 실 데이터의 길이를 ysz길이보다 최소 한개가 
        #적게하여 취급한다.

        if inplus: self.inplus_ysz = bsz #타겟에 입력값도 포함하여 목표 학습
        else: self.inplus_ysz = self.y_seq_len
        if inplus and recall_train == 0: self.iaccu_prec = self.x_seq_len
        else: self.iaccu_prec = 0
        
        if inplus and self.x_seq_len == 0:
            print('inplus option error')
            exit()
        if recall_train and inplus == 0: 
        ...
        mp.setbygate(self.trc, self.by_gate, self.y_seq_len, self.go, self.end, embedim)
        ...
        if psz > 0 and pre_train == 1:
            ...
            mp.accuracy(self.im, self.accu_input, self.accu_label, ydisc)#impluse는 accuray용 cell을 
            #따로 두므로 out gate에서 ydisc정보가 전달되지 못하므로 여기서 명시
            #따로두지않고 필요하면 ipare)와 함께 아래것으로 하여 마지막 커플의 것으로 한다.
            #self.accu_input, self.accu_label = mp.accuracy(self.im, self.accu_input, self.accu_label) 
            self.cell = None
        else:#pre_train이 0 이거나 2, 2이면 이전코드 압축을 genric으로 하여 기울기 역전파 시킨다.
            self.im = None
    ...
    def train(self, prev_ids, in_ids, tar_ids):#in_ids, tar_ids는 
        ...
        if in_ids is not None:
            if self.in_d is None: 
                ...
                mp.resizing2(self.go, self.in_d)
                mp.resizing2(self.end, self.in_d)
        ...
        if self.tar_d is None: 
            ...
            mp.resizing2(self.go, self.tar_d)
            mp.resizing2(self.end, self.tar_d)
        ...
        if self.inplus:#타겟에 입력값도 포함하여 목표 학습
            if self.recall_train == 0:#ytar: input + target + <end token>
            ...
            #else: recall_train과정이면 오토인코딩(자기구조학습)으로서 
        ...
        if self.recall_train:#입력과 타겟값을 동일하게 학습 
    ...
    def predict(self, prev_ids, in_ids, tar_ids = None):
        ...
        if self.p_seq_len != 0 and prev_ids is not None: 
            ...
            if self.bgate_bsize != prev_ids.shape[0]:#내부에서 추론과정에서 바이게이트를 사용하는데 
                ...
                mp.resizing4(self.by_gate, self.bgate_bsize) #초기상태이므로 입력에 맞게 재설정한다.
        ...
        if tar_ids is not None:
            ...
            if self.inplus:#타겟에 입력값도 포함하여 오차값 평가
                ...
                if self.recall_train:#ytar: input + <go token> + target
    ...
    def predicts(self, prev_ids, input_ids, batch_size):
        ...
        while i + batch_size <= n:
            ...
            i += batch_size
        if self.ydisc and self.pred_natural == 0: y_preds = np.array(y_preds, dtype='i')
